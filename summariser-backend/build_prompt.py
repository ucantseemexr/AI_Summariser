...
def build_prompt(text: str, sections: list[dict]) -> str:
    prompt = f"""You are a helpful summarization assistant.
Your task is to generate section-wise summaries STRICTLY based on the input text.
...
- Follow the format in the example strictly.
    ...
    ---
Now follow the same pattern generate summary.
Only summarise the content strictly based on the provided passage below. Do not include external information or assumptions.
...
=== Input Text ===
(Use only the content below to answer. Do not include anything not in the text.)
    {text}
...
Causes
Summarise: What led to this event?

=== Input Text ===
The American Civil War broke out in 1861 after decades of simmering tensions between Northern and Southern states over slavery, 
states’ rights, and economic differences. The election of Abraham Lincoln, who opposed the expansion of slavery, 
led Southern states to secede and form the Confederacy, prompting war.

=== Summary ===

The American Civil War was caused by long-standing tensions over slavery, states’ rights, and economic differences, 
culminating in the secession of Southern states following Abraham Lincoln’s election.
=== End of Output ===
...
Significance
Summarise: Why is this event important?

=== Input Text ===
The moon landing in 1969 was a pivotal moment in human history, demonstrating the capabilities of space exploration and 
marking the United States' victory in the Space Race. Neil Armstrong became the first human to walk on the moon, 
which inspired generations of scientific and technological advancement.

=== Summary ===

The 1969 moon landing symbolized a major achievement in space exploration and American innovation, inspiring future technological 
progress and demonstrating the potential of human ingenuity.
=== End of Output ===
...
Benefits
Summarise: List the benefits of a plant-based diet.

=== Input Text ===
A plant-based diet emphasizes whole grains, vegetables, fruits, legumes, nuts, and seeds while minimizing or eliminating animal products. 
Research shows that it can reduce the risk of chronic illnesses such as heart disease, type 2 diabetes, and certain cancers. 
t may also promote weight loss, improve digestion, and contribute to environmental sustainability.

=== Summary ===

A plant-based diet can reduce the risk of chronic diseases, promote healthy weight management, improve digestion, and support 
...
Risks
Summarise: List the risks of drinking alcohol.
...
Paragraph Summary
Summarise the input into one coherent paragraph.
